...
def prepare_csv(val_ratio = 0.2):
    ...
    traindf.iloc[idx[val_size:], :].to_csv('temp/d_train.csv', index=False)
    valdf.iloc[idx[:val_size], :].to_csv("temp/d_val.csv", index=False)

prepare_csv()

train_d = torchtext.data.TabularDataset.splits(
path='data/',train='d_train.csv',validation='d_validation.csv', format='csv',skip_header=True,
fields=[('labels', torchtext.data.Field()),
...
VOCAB_SIZE = len(TEXT.vocab) # len(train_d.get_vocab())
EMBED_DIM = 32
NUN_CLASS = 4
